...
int main() {
  ...
  auto set = generatePointset<int>(100, 5, gen);
  auto set2 = set;
  auto mx = std::vector<int>{1, -1, 1, -1, 1};

  std::cout << "# Before filter\n";
  printPoints(set);

  nondlib::inplace::filterQuadD(set, mx);
  ...
  auto newpoints = generatePointset<int>(100, 5, gen);
