...
public:
    ...
    Point(T c, int rank = 0, bool isA = false, bool isB = false)
        : c(c), rank(0), isA(isA), isB(isB) {}
...
void multiplyMaxima(T v, M maxima)
{
    for (size_t i = 0; i < v.size(); ++i)
    {
        ...
        { //Initial multiplication required if because of maximization/ minimization criteria
            v[i][j] *= maxima[j];
        }
    }
}
...
void filterQuadD(std::vector<C> &v, M &maxima)
{
    ...
    */
    ...
    {
        for (int rempoint = point + 1; rempoint != v.size(); rempoint++)
        { // Since we do a comparison on each side there is no need to test the points before the point
            bool dominated = true, dominator = true;
            for (int dim = 0; dim != v[point].size() && (dominated || dominator); dim++)
            { // And we verify if it respects the conditions
                if (v[point][dim] > v[rempoint][dim])
                { // If the points contains atleast 1 dimension greater than some other point it is not dominated and cant be removed
                    dominated = false;
                }
                if (v[point][dim] < v[rempoint][dim])
                { // Neither Dominated nor dominator
                    dominator = false;
                }
            }
            if (dominated)
            {                                                    // Remove the point if it is dominated
            ...
            { //Same idea but for the other point
...
void updateMaxima2D(std::vector<C> &v, M &maxima, C &point)
{
    ...
    if (idx == 0 || v[idx - 1][1] < point[1])
    {
        ...
        for (int i = nxt; i < v.size() - 1; i++)
        { // Find all elements are to be removed with this set update
            if (v[i][1] > point[0])
            {
                lst = i;
                break;
        ...
        if (lst != nxt)
        {
            for (size_t i = lst; i < v.size() - 1; i++)
            { // Copy the elements using 2 pointers to the correct location
                v[nxt][0] = v[lst][0];
                v[nxt][1] = v[lst][1];
                nxt++;
                break;
...
bool _dominated2d(std::set<std::array<T, 2>, std::greater<std::array<T, 2>>> &aux, std::vector<double> p, size_t lo, size_t hi)
{
    ...
    */
    ...
    if (it != aux.begin())
    { // Here we want to test wether the previous elemnt dominates current. In case there is no such element (aka if the element will be the first in the list) we cant do that
        auto prev = it;
        prev--;
        if ((*prev)[1] >= p[hi])
        { // Condition evaluates if the point is already dominated by some other point
            return true;
        }
    }
    ...
    auto start = it;
    while (it != aux.end())
    { // Here we loop until the end and check if there are dominated elements, then we remove them all at once
        if ((*it)[1] > p[hi])
        { //What happens here is, we use a aux var to store the deleted iterator, then there is no need to increment rit since one element is remove and it point to the next position
            break;
        }
...
void updateMaxima3D(std::vector<C> &v, M &maxima, int obj, C &point)
{
    ...
    else if (!(_dominated2d(aux, point, lo, hi)))
    { // If the point is not dominated by the current points
        v.insert(v.begin() + pos, point);
        first = pos + 1;
        valid = true;
    }
...
std::vector<Point<T>> setMaximaK(std::vector<Point<T>> &S, int dims)
{
    ...
    std::vector<std::reference_wrapper<Point<std::reference_wrapper<T>>>> aux2(
        aux.begin(), aux.end());
    ecdfk(aux2, dims);
